...
public class Movie : IMovie
{
    ...
    {
        // check if there's no available copies left
        if (this.availablecopies <= 0)
        {
            return false;
        }

        // check if there's a duplicate member
        if (borrowers.Search(member) == true)
        {
            return false;
        }

        // add member to borrowers
    ...
    public int CompareTo(IMovie another)
    {
        ...
        int compare = this.title.CompareTo(another.Title);
    ...
    {
        return "Movie: " + title + ", Genre: " + genre + ", Classification: " + classification + ", Duration: " + duration;
    }
}
